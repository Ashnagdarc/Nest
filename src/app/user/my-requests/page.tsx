            ...
            >
                ...
                <TableBody>{/* Removed potential whitespace */}
                    ...
                    <motion.tr key={req.id} variants={itemVariants}>
                        <TableCell className="font-medium">{req.gearNames.join(', ')}</TableCell>
                        <TableCell>{format(req.requestDate, 'PP')}</TableCell>
                        <TableCell>{req.duration}</TableCell>
                        <TableCell>{getStatusBadge(req.status)}</TableCell>
                         <TableCell className="text-xs">
                           {req.checkoutDate && <div>Out: {format(req.checkoutDate, 'PP')}</div>}
                           {req.dueDate && <div className={`${req.status === 'Overdue' ? 'text-destructive font-semibold' : ''}`}>Due: {format(req.dueDate, 'PP')}</div>}
                            {/* @ts-ignore TODO: Add checkinDate to mock type */}
                           {req.checkinDate && <div>In: {format(req.checkinDate, 'PP')}</div>}
                         </TableCell>
                        <TableCell className="text-xs text-muted-foreground">{req.adminNotes || 'N/A'}</TableCell>
                        <TableCell className="text-right">
                           {req.status === 'Pending' && (
                              <Button variant="outline" size="sm" onClick={() => handleCancelRequest(req.id)}>
