    ...
    try {
      ...
      try {
        const gearList = availableGears
          ...
          .map(
            (g, idx) =>
              `${idx + 1}. ${g.name} (Qty: ${data.quantities?.[g.id] || 1})`,
          )
          .join("\n");
        ...
        const bookedForDisplay =
          bookedForUser?.full_name || bookedForUser?.email || "Unknown";
          ...
          if (bookedForUser.email) {
            await fetch("/api/send-email", {
              ...
              body: JSON.stringify({
                ...
                data: {
                  recipientName:
                    bookedForUser.full_name ||
                    bookedForUser.email?.split("@")[0],
              ...
              console.error("Email to booked-for person failed:", err),
            );
          }
          await createSystemNotification(
            ...
            console.error("Push to booked-for person failed:", err),
          );
        }
          ...
          await fetch("/api/send-email", {
            ...
            body: JSON.stringify({
              ...
              data: {
                ...
                bookingFor:
                  bookingType === "other" ? bookedForDisplay : "Yourself",
        ...
        if (data.teamMembers.length > 0) {
          const teamMemberProfiles = availableUsers.filter((u) =>
            data.teamMembers.includes(u.id),
          );
          for (const member of teamMemberProfiles) {
            if (member.email) {
              await fetch("/api/send-email", {
                ...
                console.error(
                  `Email to team member ${member.email} failed:`,
                  err,
                ),
              );
            }
              ...
              body: JSON.stringify({
                ...
                data: {
                  ...
                  teamMembers: data.teamMembers.length
                    ? availableUsers
                        .filter((u) => data.teamMembers.includes(u.id))
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    I will be using this equipment personally
                  </p>
                </div>
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Booking on behalf of a colleague or team
                  </p>
                </div>
                <div className="w-8 h-8 border border-neutral-700 group-hover:border-orange-500/50 flex items-center justify-center text-muted-foreground group-hover:text-orange-400 transition-all">
                  <svg
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <span
                className={`px-3 py-1 text-xs font-medium border ${bookingType === "self" ? "bg-orange-500/20 text-orange-400 border-orange-500/30" : "bg-blue-500/20 text-blue-400 border-blue-500/30"}`}
              >
            ...
            <div className="flex items-center gap-4">
              <p className="text-muted-foreground text-lg max-w-2xl">
                {bookingType === "self"
                  ? "Select the tools you need and provide details for your project."
                  : "Fill in the details for the person or team you are booking equipment for."}
              </p>
              <button
                type="button"
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                        ...
                        filteredGears.map((g) => {
                          ...
                          return (
                            <div
                              ...
                              onClick={() => {
                                const current =
                                  form.getValues().selectedGears || [];
                                const updated = isSelected
                                  ? current.filter((id) => id !== g.id)
                                  : [...current, g.id];
                                form.setValue("selectedGears", updated, {
                                  shouldValidate: true,
                                });
                              }}
                            ...
                            >
                              <div
                                className={`w-4 h-4 border flex items-center justify-center transition-all shrink-0 ${isSelected ? "border-orange-500 bg-orange-500" : "border-neutral-600"}`}
                              >
                                {isSelected && (
                                  <svg
                                    width="10"
              ...
              <Card className="bg-neutral-900 border-neutral-800">
                <CardHeader className="pb-4 border-b border-neutral-800">
                  ...
                  <CardDescription>
                    Provide purpose and timeline for your request.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6 pt-6">
                          ...
                          return (
                            <FormItem>
                              ...
                              <p className="text-xs text-muted-foreground">
                                This person will receive an email and push
                                notification about this booking
                              </p>
                              <FormMessage />
                            </FormItem>
                          );
                          ...
                          >
                            {reasonOptions.map((reason) => (
                              <div
                                ...
                                className={`px-3 py-1.5 cursor-pointer text-xs font-medium transition-all border ${field.value === reason ? "bg-orange-500 text-white border-orange-500" : "bg-neutral-800 text-muted-foreground border-neutral-700 hover:border-orange-500/50 hover:text-foreground"}`}
                              >
                                {reason}
                              </div>
                            ))}
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            {field.value.map((id) => {
                              ...
                              return (
                                <div
                                  key={id}
                                  className="flex items-center gap-2 px-3 py-1.5 bg-neutral-800 border border-neutral-700 text-sm"
                                >
              ...
              {form.watch("selectedGears")?.length > 0 && (
                <Card className="bg-neutral-900 border-neutral-800">
                  <CardHeader className="pb-4 border-b border-neutral-800">
                    ...
                    <CardDescription>
                      {form.watch("selectedGears").length} item
                      {form.watch("selectedGears").length !== 1 ? "s" : ""}{" "}
                      selected
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4 pt-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {availableGears
                        .filter((g) =>
                          form.watch("selectedGears").includes(g.id),
                        )
                        .map((g) => {
                          ...
                          return (
                            <div
                              key={g.id}
                              className="flex items-center gap-4 p-4 bg-neutral-950 border border-neutral-800"
                            >
                                ...
                                <div className="mt-2 flex items-center justify-between">
                                  <div className="flex items-center gap-2">
                                    <button
                                      ...
                                      className="w-7 h-7 flex items-center justify-center border border-neutral-700 bg-neutral-800 hover:border-orange-500/50 transition-colors text-muted-foreground hover:text-foreground"
                                    >
                                      <svg
                                        width="12"
                                        height="12"
              ...
              <div className="flex gap-3 pt-2">
                ...
                <Button
                  ...
                  className="h-11 px-6 bg-neutral-800 border-neutral-700 hover:bg-neutral-700 hover:border-neutral-600 text-foreground"
                >
                  Cancel
                </Button>
              </div>
